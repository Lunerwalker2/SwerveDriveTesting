...
public class SwerveAuto extends SwerveCore {
    ...
    protected boolean debugActive = Boolean.FALSE;
    protected long debugStates = 1;
    protected autoStates debugStartState = autoStates.SWERVE_DONE;
...
//        vision.shutdown();
    ...
    private Boolean orientRobot(double newOrientationDegrees) {
        ...
        while (Math.abs(newOrientationDegrees - ourSwerve.curHeading) > 5.0) {
            ...
            if (Math.abs(newOrientationDegrees - ourSwerve.curHeading) > 90.0) {
                turnSpeed = 0.20;
            } else {
                turnSpeed = 0.10;
            }
            if (newOrientationDegrees > ourSwerve.curHeading) {
                turnSpeed = -turnSpeed;
            }
