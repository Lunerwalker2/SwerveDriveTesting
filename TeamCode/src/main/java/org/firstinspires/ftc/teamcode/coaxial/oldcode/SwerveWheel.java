...
public class SwerveWheel {
    ...
    //
    // ***********************************************************************
    void updatePowersPositions(double power, double position){
    ...
    public void updateWheel(double newSpeed, double newPosition, boolean errorsLoud) throws InputMismatchException {
        // be sure we have a valid new position
        if(( newPosition < -1.0 ) || ( newPosition > 1.0 )) {
            if (errorsLoud) throw new InputMismatchException("Servo position invalid!");
            newPosition = 0;
        }
        // be sure we have a valid and useful speed
        if ( newSpeed < 0 ) {
            if (errorsLoud) throw new InputMismatchException("Motor speed invalid!");
            newSpeed = 0;
        }
